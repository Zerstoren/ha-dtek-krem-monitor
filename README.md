...
1. Copy `custom_components/dtek_monitor_krem` into `config/custom_components/`.
...
Brand assets are bundled locally in `custom_components/dtek_monitor_krem/brand/`, so no external icon hosting is required.
...
1. Скопіюйте `custom_components/dtek_monitor_krem` до `config/custom_components/`.
...
Брендові ресурси постачаються локально в `custom_components/dtek_monitor_krem/brand/`, тому зовнішній хостинг іконок не потрібен.
